...
@section('title')
    @if (!is_null($tag)) {{$tag->title}} @endif
@stop
    ...
    <div class="container">
        ...
        <div class="product-group clearfix">
            ...
            <div class="button">
                <div class="paging">
                    <ul class="pagination">
                        {!! with(new App\ProductTagPaginationLinks($products))->render() !!}
                    </ul>
                </div>
                @if ($products->currentPage() < $products->lastPage())
                <a class="button-1" href="{{ url('tag/' . $productId . '-' . $tag->slug)}}/trang-{{$currentPage+1}}.html" title="">Xem thêm sản phẩm<span><i class="fa fa-angle-right"></i></span></a>
                @endif
            </div>
            @endif
        </div>
