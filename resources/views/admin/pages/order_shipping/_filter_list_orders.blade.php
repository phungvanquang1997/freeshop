<div class="row no-padding filter-container">
    ...
    <div class="form-group">
        ...
        <div class="col-sm-2">
            <input name="order_code" type="text" class=" form-control" placeholder="Tìm theo đơn hàng" value="{{ request()->has('order_code') ? request()->get('order_code') : '' }}">
        </div>
        <div class="col-sm-2">
            <input name="email" type="text" class=" form-control" placeholder="Tìm theo email khách hàng" value="{{ request()->has('email') ? request()->get('email') : '' }}">
        </div>
        ...
        <div class="col-md-5" style="margin-top: 3px;">
            ...
            @if (isset($search_type) && $search_type == 'list_orders_service')
                <a href="{{ url('admin/order-shipping-report/export_list_orders_service/' . $date . '/' . request()->get('email') ) }}" class="@if ($date == '')disabled-link @endif btn btn-success"><i class="fa fa-file-excel-o"></i> {{ trans('lang.follow_orders') }}</a>
            @endif
        </div>   
    </div>
