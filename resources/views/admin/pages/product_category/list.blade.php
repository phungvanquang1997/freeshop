    ...
    <div class="box">
        <div class="box-body">
            <div class="col-sm-2 pull-right text-right">
                <a class="btn btn-primary" href="{{ url('admin/category/product/create') }}"><i class="fa fa-plus"></i> {{trans('lang.add_new')}}</a>
            </div>
            <div class="row no-padding filter-container">
                {!! \Form::open(['method' => 'GET', 'url' => url('admin/category/product')]) !!}
                <div class="col-sm-2">
                    ...
                    @foreach ($parents as $category)
                        <tr>
                            ...
                            <td>
                                ...
                                {!! App\Helpers\MyHtml::btnRemove('admin/category/product/' . $category->id) !!}
                            </td>
                        </tr>
                        ...
                        @foreach ($category->children()->orderBy('order', 'asc')->get() as $child)
                            <tr>
                                ...
                                <td>{{ $child->order }}</td>
                                <td>
                                    {!! App\Helpers\MyHtml::btnEdit('admin/category/product/' . $child->id . '/edit/') !!}
                                    {!! App\Helpers\MyHtml::btnRemove('admin/category/product/' . $child->id) !!}
                                </td>
                            </tr>
                            ...
                            @foreach ($child->children()->orderBy('order', 'asc')->get() as $grandson)
                                <tr>
                                    ...
                                    <td>
                                        {!! App\Helpers\MyHtml::btnEdit('admin/category/product/' . $grandson->id . '/edit/') !!}                                    
                                        {!! \Form::open(['method' => 'DELETE', 'url' => 'admin/category/product/' . $grandson->id, 'class' => 'inline']) !!}
