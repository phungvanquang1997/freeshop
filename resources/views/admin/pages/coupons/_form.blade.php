...
<div class="form-group">
    ...
    <div class="col-sm-7">
        <div class="input-group date">
            <input name="date_range" type="text" class=" form-control" id="datepicker" value="{{ request()->has('date_range') ?? (isset($date_range) ? $date_range : '') }}" placeholder="">
    ...
    <div class="col-sm-7">
        <select name="status" class="form-control">
            @foreach($status as $key => $value)
                <option value="{{ $key }}" {{ (old('status') ? old('status') : (isset($coupon) && $coupon->status == $key)) ? 'selected' : '' }}>{{ $value }}</option>
            @endforeach
        </select>
    </div>
</div>
